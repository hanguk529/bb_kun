...
- **도구 구성 원칙**: 가능한 한 **Windows 내장 명령어와 Sysinternals Suite**로 해결한다. 외부 GUI 도구는 설치/서명/관리자 정책 문제로 막히는 경우가 많고, 어차피 본질은 같은 API를 호출하는 wrapper다. 정적 분석과 동적 후킹만 외부 도구를 쓴다.
  - **Sysinternals Suite**: `https://learn.microsoft.com/sysinternals/downloads/sysinternals-suite` — Process Explorer, Procmon, TCPView, WinObj, AccessChk, Listdlls, Sigcheck 일괄 포함.
  - **Windows 내장**: `netsh`, `sc`, `icacls`, `pktmon`, `tasklist`, `findstr`, PowerShell — 별도 설치 불필요.
  - **Frida**: `pip install frida-tools` — 동적 후킹은 대체 불가.
  - **Ghidra**: `https://github.com/NationalSecurityAgency/ghidra/releases` — 정적 분석용. (대안: 무료 IDA Free, Cutter)
  - **Wireshark (선택)**: `https://www.wireshark.org/download.html` — Pktmon ETL 분석에 활용 가능. 안 깔려도 Pktmon만으로 1차 분석은 된다.
...
**도구**: Sysinternals Procmon, AccessChk, WinObj, Handle / Windows 내장 PowerShell, `sc`
...
### 2.2. RPC 엔드포인트 추적 (내장 도구)

고도화된 ZTNA는 파이프 대신 Microsoft RPC(ALPC 기반)를 사용한다. RpcView는 비공식 도구라 환경에 따라 안 도는 경우가 많은데, 내장 명령어와 Sysinternals 조합으로 거의 동일한 정보를 뽑을 수 있다.

#### 2.2.1. ALPC 포트 열거 (Sysinternals Handle)

10. ZTNA 데몬이 열고 있는 핸들을 전부 덤프한다.

    ```cmd
    handle64.exe -accepteula -p <PID> -a > handles.txt
    ```

    `-a`는 모든 객체 타입을 보여준다.

11. `handles.txt`에서 ALPC 포트와 RPC 관련 객체만 추출한다.

    ```cmd
    findstr /I "ALPC Port \\RPC" handles.txt
    ```

12. 출력 예시:

    ```
    1A4: ALPC Port    \RPC Control\OLEABCD1234.0
    2B8: ALPC Port    ZTNA_RPC_Control_v1
    ```

    이름이 붙은 ALPC 포트(`ZTNA_RPC_Control_v1` 등)가 RPC 인터페이스의 입구다. **이름이 정적이면 일반 유저로 직접 연결 시도가 가능**하다.

#### 2.2.2. RPC 엔드포인트 매퍼 조회 (PowerShell)

13. 현재 시스템에 등록된 모든 RPC 엔드포인트를 쿼리한다. PowerShell을 관리자 권한으로 열고:

    ```powershell
    # rpcdump.exe (Sysinternals) 가 가장 깔끔하지만 미포함인 경우 PortQry 또는 아래 .NET 사용
    Get-Process | Where-Object { $_.Name -like "*ZTNA*" } | ForEach-Object {
        Write-Host "=== $($_.Name) (PID: $($_.Id)) ==="
        # 프로세스가 등록한 RPC 인터페이스 간접 확인
        $_.Modules | Where-Object { $_.ModuleName -like "rpcrt4*" -or $_.ModuleName -like "*rpc*" }
    }
    ```

14. 더 직접적인 방법 — `WinObj` (Sysinternals)로 RPC 객체 디렉터리를 본다.

    - `winobj.exe`를 관리자 권한으로 실행.
    - 좌측 트리에서 `\RPC Control` 디렉터리로 이동.
    - ZTNA 벤더명을 포함한 ALPC 포트 객체를 찾는다.
    - 객체 우클릭 → `Properties` → `Security` 탭에서 DACL 확인. 일반 유저에게 `Connect` 권한이 있으면 타겟이다.

#### 2.2.3. RPC 인터페이스 UUID 추출 (Ghidra 정적 분석)

15. RpcView 없이 UUID를 얻는 가장 확실한 방법은 데몬 바이너리를 Ghidra로 여는 것이다.
16. Ghidra의 `Search` → `For Strings`에서 GUID 패턴을 검색하거나, `Symbol Table`에서 다음 함수 xref를 본다.
    - `RpcServerRegisterIf` / `RpcServerRegisterIf2` / `RpcServerRegisterIfEx`
    - `RpcServerUseProtseqEp`
17. 함수의 첫 번째 인자가 `RPC_SERVER_INTERFACE` 구조체 포인터다. 구조체 시작 부분의 16바이트가 인터페이스 UUID, 그다음이 버전이다.
18. UUID와 노출된 메서드(함수) 번호를 표로 정리한다.

    | UUID | Protocol Sequence | Endpoint | # of Procedures | 인증 요구 |
    |------|------|------|------|------|
    | `12345678-...` | `ncalrpc` | `ZTNA_RPC_Control_v1` | 23 | None |

#### 2.2.4. RPC 포트 보안 검증

19. 식별된 ALPC 포트가 일반 유저로 열리는지 확인한다.

    ```cmd
    accesschk64.exe -accepteula -o "\RPC Control\<ALPC포트이름>"
    ```

    `-o`는 객체 보안 정보를 출력한다. `BUILTIN\Users`나 `Everyone`에 `Connect`/`All Access`가 있으면 **즉시 타겟.**
...
**도구**: Sysinternals TCPView, Windows 내장 Pktmon / netsh wfp, Wireshark (선택)
...
### 3.2. 패킷 캡처 (Pktmon — Windows 내장)

Windows 10 1809 / Server 2019부터 `pktmon.exe`가 내장되어 있다. Wireshark 설치 없이도 ETW 기반 캡처가 가능하다.

6. 관리자 cmd에서 캡처 대상 필터를 등록한다.

    ```cmd
    :: 게이트웨이 IP만 캡처
    pktmon filter add -i <게이트웨이IP>

    :: 또는 특정 포트
    pktmon filter add -p 443
    pktmon filter add -p 1194

    :: 등록된 필터 확인
    pktmon filter list
    ```

7. 캡처 시작 (실시간 표시 + 파일 저장):

    ```cmd
    pktmon start --capture --pkt-size 0 --file-size 100 -f capture.etl
    ```

    - `--pkt-size 0`: 페이로드 전체 캡처(기본 128바이트로 잘림)
    - `--file-size 100`: 100MB까지 순환 기록

8. ZTNA 통신을 트리거(서비스 재시작, UI Reconnect 등) 후 캡처 중지:

    ```cmd
    pktmon stop
    ```

9. ETL 파일을 텍스트로 변환해 빠르게 본다.

    ```cmd
    pktmon etl2txt capture.etl -o capture.txt
    ```

    더 자세히 보려면 pcapng로 변환해서 Wireshark에 넣는다.
...
### 3.2-bis. Wireshark로 심층 분석 (선택)

위에서 생성한 `capture.pcapng`를 Wireshark에서 열거나, 직접 캡처해도 된다. 필터링:
...
판단:
- **TLS만 보임**: TCP 위 TLS. mitmproxy로 인터셉트 검토 가능.
- **QUIC 보임**: HTTP/3 가능성. mitmproxy QUIC 지원 버전 필요.
- **WireGuard 보임**: UDP. **HTTP 프록시 기반 인터셉트는 즉각 폐기.** 별도 키 추출 + 자체 디코더 필요.
...
### 3.3. WFP 콜아웃 식별 (`netsh wfp` — Windows 내장)

WFP(Windows Filtering Platform)는 커널 레벨에서 트래픽을 가로채는 정식 프레임워크다. ZTNA가 여기에 콜아웃을 등록했다면 트래픽이 유저 모드를 거치지 않고 캡슐화된다는 의미다.

WFP Explorer는 비공식 GUI라 환경에 따라 설치/실행이 막힌다. **Windows 7부터 내장된 `netsh wfp` 명령**으로 동일한 정보를 더 빠르게 얻을 수 있다.

#### 3.3.1. WFP 전체 상태 덤프

11. 관리자 cmd에서 WFP 상태를 통째로 XML로 떠낸다.
    ...
    이 파일에는 다음이 모두 들어 있다.
    - 등록된 **Provider** (어느 드라이버가 WFP에 자기 자신을 등록했는지)
    - 등록된 **Callout** (어느 함수가 어느 layer에 콜아웃으로 걸려 있는지)
    - 등록된 **Filter** (어떤 조건에서 콜아웃을 호출하는지)
    - **Sublayer** 우선순위

#### 3.3.2. ZTNA 벤더 식별

12. XML에서 ZTNA 벤더 키워드를 검색한다.

    ```cmd
    findstr /I /N "zscaler crowdstrike netskope paloalto cisco" wfp_state.xml
    ```

13. 매치된 라인의 위아래 컨텍스트를 본다. 다음 정보를 추출한다.
    - `<providerKey>` 안의 GUID → 이 Provider가 ZTNA의 WFP 진입점.
    - 같은 Provider를 참조하는 `<callout>` 요소들 → ZTNA가 후킹하는 layer 목록.
    - `<applicableLayer>` 값 → `FWPM_LAYER_ALE_AUTH_CONNECT_V4`, `FWPM_LAYER_STREAM_V4` 등.

14. Provider 요소에는 `<serviceName>`이나 `<displayData><name>`이 같이 있어 어떤 드라이버 서비스인지 바로 알 수 있다.
...
16. ZTNA Provider GUID로 필터를 좁힌다.

    ```cmd
    findstr /I /C:"<providerKey>{ZTNA-PROVIDER-GUID}" filters.xml
    ```

    각 필터의 `<conditions>` 블록이 "어떤 트래픽일 때 콜아웃을 호출하는지"의 정답이다.

#### 3.3.4. 실시간 매칭 추적 (선택)

17. 어떤 필터가 실제로 트래픽을 잡고 있는지 보려면 진단 캡처를 쓴다.

    ```cmd
    netsh wfp capture start file=wfp_capture.cab
    :: ...ZTNA 통신 트리거...
    netsh wfp capture stop
    ```

    생성된 `.cab`를 풀면 `wfpdiag.xml`이 나오고, 거기에 어떤 패킷이 어떤 필터에 매칭되어 어떤 콜아웃으로 분기되었는지 기록되어 있다.

#### 3.3.5. 등록 드라이버 파일 식별

18. Provider의 `<serviceName>`을 가지고 실제 `.sys` 파일 경로를 추적한다.

    ```cmd
    sc qc <ServiceName>
    ```

    `BINARY_PATH_NAME` 항목이 드라이버 경로다.

19. 또는 한 줄로 ZTNA 관련 커널 드라이버를 전부 본다.

    ```cmd
    driverquery /v /fo csv | findstr /I "zscaler crowdstrike netskope <벤더>"
    ```

20. Sysinternals `sigcheck`로 서명 정보까지 한 번에 확인한다.

    ```cmd
    sigcheck64.exe -accepteula -a -h "C:\Windows\System32\drivers\<드라이버>.sys"
    ```

### 3.4. 커널 드라이버 정보 확인

21. `.sys` 파일에 대해 서비스 구성과 서명을 확인한다.

    ```cmd
    :: 서비스 시작 모드 / 의존성
    sc qc <서비스명>

    :: 서명자 / 해시 / 카탈로그 검증 (Sysinternals)
    sigcheck64.exe -accepteula -a -h -i C:\Windows\System32\drivers\<드라이버>.sys
    ```

    `-i` 옵션으로 카탈로그 서명까지 검증된다. Windows 내장 `signtool`은 SDK 설치가 필요해서 sigcheck를 권장한다.

22. 드라이버가 IOCTL 인터페이스를 열고 있다면, `IoCreateDevice` 호출의 SDDL을 Ghidra로 확인한다. 일반 유저가 `DeviceIoControl`을 호출할 수 있으면 **커널 공격 표면**이다.

23. 디바이스 객체 권한은 WinObj로 즉시 확인 가능하다.

    - `winobj.exe` 관리자 권한 실행.
    - `\Device` 디렉터리에서 ZTNA 관련 디바이스 객체를 찾는다.
    - 우클릭 → `Properties` → `Security` 탭. `BUILTIN\Users`에 `Read/Write` 허용이면 일반 유저가 `\\.\<DeviceName>`로 직접 IOCTL을 쏠 수 있다.
